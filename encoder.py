...
class Encoder(tf.keras.layers.Layer):
    ...
    def call(self,inputs):
        ...
        batch_tr_ans_tokens:[batch_size,num_ans,max_ans_len] int32
        batch_te_ans_tokens:[batch_size,num_ans,max_ans_len] int32
        batch_padding_mask:[batch_size,max_qst_len] float32  
        """
        batch_qst_tokens,batch_tr_img_ids,batch_te_img_ids,batch_tr_ans_tokens,batch_te_ans_tokens = inputs
        qst_enc = self.model(batch_qst_tokens)#[batch_size,max_qst_len,768] float32
        ...
        tr_ans_enc = self.model(batch_tr_ans_tokens)#[batch_size,num_ans,max_ans_len,768]
        te_ans_enc = self.model(batch_te_ans_tokens)#[batch_size,num_ans,max_ans_len,768]
        tr_ans_enc = tf.reduce_mean(tf.reduce_sum(tr_ans_enc,axis = 2),axis = 1)#[batch_size,768]
        te_ans_enc = tf.reduce_mean(tf.reduce_sum(te_ans_enc,axis = 2),axis = 1)#[batch_size,768]
